 ...
 *                          1.   Open file 'empdata.txt' and create Employee LinkedList
 ...
 * Class:       EmployeeList
 * 
 *<p>Contains Employee OrderedDLinkedList which is created by
 *the constructor, and configured by class methods
****************************************************************/
public class EmployeeList {
    /*****************************************************************
     * Employee OrderedDLinkedList to store and configure employees
     *****************************************************************/
    private OrderedDLinkedList<Employee> empList;
     ...
     * 
     *<p>Creates OrderedDLinkdList<Employee> object and
    ...
    EmployeeList() throws IOException, IllegalArgumentException {
        //create Employee OrderedDLinkedList
        empList = new OrderedDLinkedList<>();  
        try {
     ...
     *              
     ***********************************************************************************************/
    private Node<Employee> searchList() {
        /**
         * s
         */
        String searchKey = "-1";             
        ...
        do {
            try {
                ...
                searchKey = terminalScanner.nextLine();         
                //check if user input passes boundary check:
                if (Integer.parseInt(searchKey) > 999) {
                    throw new IllegalArgumentException("ERROR: USER INPUT IS GREATER THAN 3 DIGITS. PLEASE KEEP DIGITS TO 3 OR LESS.");
                }
                if (Integer.parseInt(searchKey) < 0) {
                    throw new Exception("ERROR: INPUT CONTAINS NON-NATURAL NUMBERS. ENSURE THAT ONLY NATURAL NUMBERS ARE ENTERED.\n(EX: 1, 2, 3...)");
                }
        ...
        }while (Integer.parseInt(searchKey) < 1);
        // iterate through list and compare IDs of employees to search value
        Node<Employee> curNode = empList.getHeadNode();

        while (curNode != null) {

            Employee e = curNode.data;

            // if theres a match, print employee details and return employee
            if (searchKey.compareTo(e.getID()) == 0) {
    ...
    public void removeEmp() {
        clearConsole();
        Node<Employee> remNode = searchList();
        if (remNode != null) {
            empList.remove(remNode);
            System.out.println("Employee succesfully removed from system");
        }
